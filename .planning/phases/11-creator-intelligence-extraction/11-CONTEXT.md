...
- **D-18:** New server action `trackSearchedCreator(channelId, channelTitle, channelUrl, thumbnailUrl)` in `creator-actions.ts`. Inserts directly into `creators` with `channel_id` already set (bypasses URL validation and the URL-resolution step in the pipeline). Then inserts `user_creators` row.
